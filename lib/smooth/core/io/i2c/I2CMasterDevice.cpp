...
bool I2CMasterDevice::read(uint8_t address, uint8_t slave_register,
                           ...
                           bool use_restart_signal, bool end_with_nack) {
  I2CCommandLink link(*this);
  ...
  if (use_restart_signal) {
    ...
    // Finish the transmission without releasing the lock we have on the i2c
    // master.
    res |= i2c_master_stop(link);
    res |= i2c_master_cmd_begin(port, link, to_tick(timeout));
  ...
  if (res != ESP_OK) {
    std::stringstream ss;
    // ss << "Error during read of address 0x" << std::hex
...
void I2CMasterDevice::scan_i2c_bus(std::vector<uint8_t>& found_devices) const {
  // Write the address of each possible device and see if an ACK is received or
  // not.
...
void I2CMasterDevice::log_error(esp_err_t err, const char* msg) {
  if (err == ESP_ERR_INVALID_ARG) {
    ...
    Log::error(log_tag, "{} - I2C driver not installed or not in master mode",
               msg);
